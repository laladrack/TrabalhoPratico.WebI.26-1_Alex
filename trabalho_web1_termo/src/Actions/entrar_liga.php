...
try {
    ...
    $stmt_membro = $pdo->prepare("SELECT liga_id FROM liga_membros WHERE liga_id = :liga_id AND usuario_id = :usuario_id");
    ...
    $stmt_inserir = $pdo->prepare("INSERT INTO liga_membros (liga_id, usuario_id) VALUES (:liga_id, :usuario_id)");
    $stmt_inserir->execute([
        ':liga_id' => $liga_id,
        ':usuario_id' => $usuario_id
    ]);

    header("Location: ../../public/dashboard.php?sucesso=Você entrou na liga com sucesso!");
    exit;

} catch (PDOException $e) {
    header('Location: ../../public/dashboard.php?erro=Erro ao processar solicitação de entrada.');
    exit;
}